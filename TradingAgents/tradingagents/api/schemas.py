...
class AnalyzeRequest(BaseModel):
    ticker: str = Field(
        ...,
        description="股票代码，如 NVDA, 600519.SH, 00700.HK",
        examples=["NVDA", "600519.SH"],
    )
    trade_date: str = Field(
        ..., description="交易日期 YYYY-MM-DD", examples=["2026-04-30"]
    )
    market_profile: MarketProfile = Field(
        default=MarketProfile.US, description="市场类型"
    )
    research_depth: ResearchDepth = Field(
        default=ResearchDepth.MEDIUM, description="研究深度"
    )
    ...
    llm_provider: LLMProvider = Field(
        default=LLMProvider.OPENAI, description="LLM 供应商"
    )
    ...
    api_key: Optional[str] = Field(
        default=None, description="LLM API Key（可选，留空则使用环境变量）"
    )
    backend_url: Optional[str] = Field(
        default=None, description="LLM 代理地址（Base URL）"
    )
    ...
    max_risk_discuss_rounds: int = Field(
        default=1, ge=1, le=5, description="最大风险讨论轮数"
    )
    # Provider-specific thinking configuration
    openai_reasoning_effort: Optional[str] = Field(
        default=None, description="OpenAI reasoning effort: low/medium/high"
    )
    anthropic_effort: Optional[str] = Field(
        default=None, description="Anthropic effort: low/medium/high"
    )
    google_thinking_level: Optional[str] = Field(
        default=None, description="Google thinking level: high/minimal"
    )
