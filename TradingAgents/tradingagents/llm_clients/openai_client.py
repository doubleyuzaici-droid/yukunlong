...
class DeepSeekChatOpenAI(NormalizedChatOpenAI):
    ...
    3. **Thinking mode control.** When ``deepseek_thinking`` is set to
       ``"enabled"`` or ``"disabled"``, the ``thinking.type`` parameter
       is injected into the request body.
    """

    deepseek_thinking: Optional[str] = None

    def __init__(self, deepseek_thinking: Optional[str] = None, **kwargs):
    ...
    def _get_request_payload(self, input_, *, stop=None, **kwargs):
        ...
        if self.deepseek_thinking and self.deepseek_thinking in ("enabled", "disabled"):
            payload.setdefault("extra_body", {})
            payload["extra_body"]["thinking"] = {"type": self.deepseek_thinking}
        return payload
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        if self.provider == "deepseek" and "deepseek_thinking" in self.kwargs:
            llm_kwargs["deepseek_thinking"] = self.kwargs["deepseek_thinking"]
